...
public class FourthOption {
    ...
    public static TreeMap<String, String> readColumn2(Integer column) {
        ...
        TreeMap<String, Integer> duplicatesMap = new TreeMap<>();
        ...
        try (Stream<String> stream = Files.lines(Paths.get("airports.dat"))) {
            stream.forEach(line -> {
                ...
                if (duplicatesMap.containsKey(neededColumn)) {
                    duplicatesMap.put(neededColumn, duplicatesMap.get(neededColumn) + 1);
                    airportsMap.put(neededColumn + duplicatesMap.get(neededColumn), line);

                }
                else {
                    duplicatesMap.put(neededColumn, 0);
